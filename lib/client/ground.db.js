...
Ground.Collection = class GroundCollection {
  ...
  update(selector, ...args) {
    const result = this._collection.update(selector, ...args);
    if(args && args.length>0 && args[args.length-1].multi) {
      this._collection.find(selector).forEach(function(doc){
        this.saveDocument(doc);
      });
    } else {
      this.saveDocument(this._collection.findOne(selector));  
  ...
  remove(selector, ...args) {
    ...
    //BT remove all documents matching selector
    const self = this;
    this._collection.find(selector).forEach(function(doc){
      doc && self.saveDocument(doc, true);
    });
